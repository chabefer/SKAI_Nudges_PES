...
MetaNudgesPES <- dbReadTable(HdF,"MetaPES") %>%
              mutate(
                Treatment = factor(Treatment,levels=c("All Nudges (Est.)","Simple Letter","All (France)","Simple Nudge vs Simple Letter","Elaborate Nudge vs Simple Letter")),
...
chabe <- chabe %>%
          ...
          mutate(
            Treatment= recode(Treatment,"All"="All (France)"), #it would be more correct to generate CE for the simple letter (it would be smaller)
            Treatment = factor(Treatment,levels=c("All Nudges (Est.)","Simple Letter","All (France)","Simple Nudge vs Simple Letter","Elaborate Nudge vs Simple Letter")),
...
ggplot(filter(MetaNudgesPES,Type=="Cost-Effectiveness",Treatment=="Simple Letter" | Treatment=="All (France)"),aes(Treatment,Effects,group=Method,color=Method)) +
  geom_pointrange(aes(ymin=Effects-1.96*SeEffects,ymax=Effects+1.96*SeEffects),position=position_dodge(0.7))+
...
The main results of this literature are as follows:
  ...
  The estimated cost-effectiveness in the region where this treatment works best moves to `r MetaNudgesPES %>% filter(Type=='Cost-Effectiveness',Treatment=='All (France)',Method=='PEESE') %>% pull(Effects) %>% round(.,0)` $\pm$ `r MetaNudgesPES %>% filter(Type=='Cost-Effectiveness',Treatment=='All (France)',Method=='PEESE') %>% pull(SeEffects) %>% round(.,0)` USD per additional contract (contrast with the estimates ignoring diffusion effects: `r MetaNudgesPES %>% filter(Type=='Cost-Effectiveness',Treatment=='Simple Letter',Method=='PEESE') %>% pull(Effects) %>% round(.,0)` $\pm$ `r MetaNudgesPES %>% filter(Type=='Cost-Effectiveness',Treatment=='Simple Letter',Method=='PEESE') %>% pull(SeEffects) %>% round(.,0)` USD per additional contract).
  More results on larger datasets are needed in order to pin down the real magnitude of cost savings.
...
**it would be more correct to generate CE with diffusion effects in France for the simple letter (it would be smaller)**
**ask data from wallander and czap to study diffusion effects** 
